...
library(ggExtra)
library(dplyr)
random_samples <- sample_n(cleaned_data, 100, replace = FALSE)

Costs <- random_samples$`Total Costs`
Charges <- random_samples$`Total Charges`

g <- ggplot(random_samples, aes(Costs, Charges, howdy)) +
    geom_count(size = 1) +
    geom_smooth(method = "lm", se = F, color = "purple") +
    theme(legend.position = "none", plot.title = element_text(hjust = 0.5)) +
    ggtitle("Total Costs vs Total Charges for 100 Patients")
...
```{r warning = FALSE}
#| echo: false
ggplot(cleaned_data, aes(x = `Total Costs`)) +
    geom_density(fill = "blue", alpha = 0.5) +
    facet_grid(`Age Group` ~ .)
labs(title = "Density Plot Example", x = "X-axis Label", y = "Density")
